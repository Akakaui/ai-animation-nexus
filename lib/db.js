// lib/db.js — Upstash Redis adapter (works on Render, Railway, any Node host)
// Falls back gracefully when Redis env vars are not set (local dev).
...
// If Redis creds are missing (local dev without .env), fall back to an in-memory store
// so the server keeps running. Data won't persist across restarts in this mode.
let kv;
if (REDIS_URL && REDIS_TOKEN) {
  kv = new Redis({ url: REDIS_URL, token: REDIS_TOKEN });
} else {
  console.warn('[DB] No Redis credentials found — using in-memory store (local dev only).');
  const _store = {};
  kv = {
    get:    async (k)    => _store[k] ?? null,
    set:    async (k, v) => { _store[k] = v; },
  };
}
